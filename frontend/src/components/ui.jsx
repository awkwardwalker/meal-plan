...
const NAV_ITEMS = [
  ...
  { path: '/schedule', label: 'Planner', icon: '✨' },
  ...
  return (
    <nav style={{
      ...
      <Link to="/" style={{ display: 'flex', alignItems: 'center', gap: '10px', flexShrink: 0 }}>
        ...
        <span style={{ fontFamily: 'Playfair Display, serif', fontSize: '16px', color: tokens.green }}>Family Meals</span>
      </Link>
        ...
        {NAV_ITEMS.map(({ path, label, icon }) => {
          ...
          return (
            <Link key={path} to={path} style={{
              ...
              <span>{icon}</span>
              <span style={{ '@media(max-width:500px)': { display: 'none' } }}>{label}</span>
            </Link>
          );
        })}
      </div>
...
export function PageHeader({ title, subtitle, action }) {
  return (
    <div style={{
      ...
      <div>
        <div style={{ fontSize: '11px', letterSpacing: '3px', textTransform: 'uppercase', color: tokens.green, fontWeight: '600', marginBottom: '6px' }}>
          Family Meal Planner
        </div>
        <h1 style={{ fontFamily: 'Playfair Display, serif', fontSize: 'clamp(24px, 4vw, 36px)', fontWeight: '400', color: tokens.text, lineHeight: '1.2' }}>
          {title}
        </h1>
